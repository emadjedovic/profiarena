...
const listenForEmailNotifications = async () => {
    try {
        ...
        client.on('notification', async (msg) => {
            ...
            const resTalent = await client.query('SELECT email FROM "User" WHERE id = $1', [talent_id]);
            const resHR = await client.query('SELECT email FROM "User" WHERE id = $1', [hr_id]);
            ...
            await sendEmail(talentEmail, subject, text, hr_id, talent_id, interview_id);

            // Optionally send a copy to HR
            await sendEmail(hrEmail, `Copy: ${subject}`, text, hr_id, hr_id, interview_id);
        });
